...
- `from` и `to` передаются в API без проверки взаимного порядка: по документации MAX `from` задаёт время, до которого запрашиваются сообщения с начала чата, а `to` — время, начиная с которого запрашиваются сообщения до конца чата
